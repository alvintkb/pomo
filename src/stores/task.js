    ...
    const selectTask = async (taskId) => {
        ...
        if (!selectedTaskIds.value.includes(taskId)) {
            ...
            try {
                ...
                tasks.value = tasks.value.map(task => task.id === taskId ? { ...task, attributes: { ...task.attributes, selected: true } } : task);
            }
            catch (error) {
    ...
    const unselectTask = async (taskId) => {
        ...
        if (selectedTaskIds.value.includes(taskId)) {
            ...
            try {
                ...
                tasks.value = tasks.value.map(task => task.id === taskId ? { ...task, attributes: { ...task.attributes, selected: false } } : task);
            }
            catch (error) {
                console.error('[TaskStore] Failed to update task selection:', error);
                selectedTaskIds.value.push(taskId);
